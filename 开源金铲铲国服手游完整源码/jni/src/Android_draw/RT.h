...
// UTF8转换函数
void getUTF8(char* buf, unsigned long long namepy) {
    UTF16 buf16[16] = {0};
    driver->read(namepy, buf16, sizeof(buf16));

    UTF16* pTempUTF16 = buf16;
    UTF8* pTempUTF8 = buf;
    UTF8* pUTF8End = pTempUTF8 + 32;

    while (pTempUTF16 < pTempUTF16 + 16) {
