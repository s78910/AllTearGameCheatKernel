...
/* 
   ...
   // 写入D类内存 void writedword(unsigned int addr, int data) {
...
long getbss(const char *name)
{
	...
	while (fgets(tmp, sizeof(tmp), fp) != NULL)  // 使用fgets返回值而不是feof
	{
		if (cnt == 1)
		{
...
Vector3A CalcAngle(Vector3A D, Vector3A W)
{
    float x = W.X - D.X;
    float y = W.Y - D.Y;
    float z = W.Z - D.Z;
