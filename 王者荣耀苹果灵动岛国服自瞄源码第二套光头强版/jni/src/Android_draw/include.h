...
#include <stb_image.h>
#include "共享.h"
#include <atomic>      // 录屏功能需要
#include <thread>      // 录屏功能需要
#include <chrono>      // 录屏功能需要
...
static bool 实体技能 = true;
static bool 顶上技能 = false;
static bool 上帝 = false;
static bool 有人解控 = false;
...
static bool unlock1 = true;  // 锁定参数(防误触)
static bool 惩戒判断 = false;
static bool 智能不惩 = false;
static bool 解控判断 = false;
...
// 录屏功能变量
static float 录屏时长 = 60.0f;  // 默认录屏时长60秒
static std::atomic<bool> isRecording(false);  // 录制状态
static std::atomic<int> recordingTimeElapsed(0);  // 录制经过的时间（秒）
...
ImColor 自瞄锁定方框颜色 = ImColor(255,0,0,255);//自瞄锁定目标方框颜色（默认红色）
ImColor 自瞄锁定射线颜色 = ImColor(255,0,0,255);//自瞄锁定目标射线颜色（默认红色）
};
...
// int FPS=120;  // 已移除，使用draw.h中的全局float FPS
...
// 注意：AndroidImgui.h 中的 TextureInfo 用于 VK 渲染
// 此处定义 TextureInfo_Old 用于旧的 OpenGL 纹理代码
struct TextureInfo_Old
{
...
TextureInfo_Old createTexture1(char* ImagePath)
{
	TextureInfo_Old textureInfo;
	int w,h,n;
...
// VK兼容的纹理加载函数声明（实现在draw.cpp中）
TextureInfoa createTexturePNGFromMem_VK(const unsigned char *buf, int len);

// 重定向到VK兼容版本
#define createTexturePNGFromMem createTexturePNGFromMem_VK
...
#include "Gerpicture/picture_obj151_png.h"
...
#include "Gerpicture/picture_obj621_png.h"
...
HeadPortrait[620] = createTexturePNGFromMem(picture_obj150_PNG_H, sizeof(picture_obj150_PNG_H));
HeadPortrait[621] = createTexturePNGFromMem(picture_obj621_PNG_H, sizeof(picture_obj621_PNG_H));
...
#include "Gerpicture/picture_obj80117_png.h"
...
#include "Gerpicture/picture_obj15110_png.h"
...
#include "Gerpicture/picture_obj15120_png.h"
...
#include "Gerpicture/picture_obj15130_png.h"
...
struct 全部头像
{
	TextureInfo_Old 头像[650];
	
};
全部头像 贴图;

TextureInfo_Old 头像贴图(int 值)
{
	return 贴图.头像[值];
}
...
void 绘制头像(TextureInfo_Old textureInfo,int x, int y)
{
	ImGui::GetBackgroundDrawList()->AddImage(textureInfo.textureId, ImVec2(x-textureInfo.w/2, y -textureInfo.h/2), ImVec2(x-textureInfo.w/2 + textureInfo.w/2.55, y-textureInfo.h/2 + textureInfo.h/2.55));
}

void 方框头像(TextureInfo_Old textureInfo,int x, int y)
{
	ImGui::GetBackgroundDrawList()->AddImage(textureInfo.textureId, ImVec2(x-textureInfo.w/2, y -textureInfo.h/2), ImVec2(x-textureInfo.w/2 + textureInfo.w/2.55, y-textureInfo.h/2 + textureInfo.h/2.55));
}

void 绘制技能(TextureInfo_Old textureInfo,int x,int y,int 大招CD,int 技能CD,int 回城判断)
{
	ImGui::GetBackgroundDrawList()->AddImage(textureInfo.textureId,ImVec2(x-textureInfo.w/2,y-textureInfo.h/2),ImVec2(x-textureInfo.w/2+textureInfo.w/2,y-textureInfo.h/2+textureInfo.h/2));
