...
#define AY_OBFUSCATE_DEFAULT_KEY ty::generate_key(AY_LINE)
#endif
...
// ty::obfuscated_data object with global lifetime that has functions for
...
// reference to a ty::obfuscated_data object with global lifetime that has
// functions for decrypting the string and is also implicitly convertable to a
// char*
#define AY_OBFUSCATE_KEY(data, key) \
	[]() -> ty::obfuscated_data<sizeof(data)/sizeof(data[0]), key>& { \
		static_assert(sizeof(decltype(key)) == sizeof(ty::key_type), "key must be a 64 bit unsigned integer"); \
		static_assert((key) >= (1ull << 56), "key must span all 8 bytes"); \
		constexpr auto n = sizeof(data)/sizeof(data[0]); \
		constexpr auto obfuscator = ty::make_obfuscator<n, key>(data); \
		thread_local auto obfuscated_data = ty::obfuscated_data<n, key>(obfuscator); \
